...
const dismissKey = (id: string) => `flouwell:workshop_dismissed:${id}`;
const todayStr = () => new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Jakarta" });

export function WorkshopBanner({ workshops }: { workshops: BannerData[] }) {
  const [visible, setVisible] = useState<BannerData[]>([]);

  useEffect(() => {
    if (typeof localStorage === "undefined") { setVisible(workshops); return; }
    const today = todayStr();
    const filtered = workshops.filter((w) => localStorage.getItem(dismissKey(w.id)) !== today);
    setVisible(filtered);
  }, [workshops]);
  ...
  const dismiss = (id: string) => {
    ...
    setVisible((v) => v.filter((w) => w.id !== id));
  };
        ...
        return (
          <div key={w.id} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-400 via-orange-400 to-rose-400 p-4 text-white shadow-lg ring-1 ring-white/20">
            ...
            <button
              ...
              className="absolute right-2 top-2 rounded-full bg-black/15 px-2 py-0.5 text-xs font-medium text-white/80 hover:bg-black/25"
              aria-label="Tutup"
            >×</button>
            <div className="relative flex items-start gap-3">
              <div className="flex flex-col gap-1">
                <span className="w-fit rounded-full bg-white/25 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">{w.badge}</span>
                ...
                <Link href={`/workshop/${w.id}`} className="mt-1 w-fit rounded-full bg-white/95 px-3 py-1.5 text-xs font-bold text-orange-600 hover:bg-white">{w.cta}</Link>
              </div>
            </div>
          </div>
        );
